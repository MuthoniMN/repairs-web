...
export default function JobItem({ job }: { job: IJob }) {
    return (
        <div className="flex flex-col gap-4 w-full shadow-md rounded-md py-4 px-6 justify-between">
            <div className="flex justify-between items-center">
                <div className="flex flex-col gap-[1px]">
                    <h3 className="text-lg font-semibold">{job.title}</h3>
                    <p className="text-sm text-slate-500">{job.slug}</p>
                </div>
                <div className={` p-2 flex justify-center items-center font-medium text-xs
                    ...
                    `}>
                    <p>{job.status.replace(job.status[0], job.status[0].toUpperCase())}</p>
                </div>
            </div>
            <p>{job.description}</p>
            <div className="flex justify-between items-center divide-x-[2px] divide-slate-100">
                <div className="w-3/10 flex flex-col h-full justify-between items-center gap-2">
